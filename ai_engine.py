...
def _get_mode_instruction(mode: str) -> str:
    ...
    # Short style reminder — prepended to all modes
    # Keeps style alive without overpowering mode rules
    style_mode_bridge = """
STYLE + MODE NOTE:
...
def _get_first_turn_instruction(mode: str) -> str:
    """
    Returns an extra reminder for the very first student message.
    This helps the model understand no prior student answer exists yet.
    """
    instructions = {
        "socratic": (
...
7. If the user asks about math, you may use LaTeX with $ or $$.
8. Do not use <br> tags.
9. Keep the answer readable and natural.
10. If the user is greeting, thanking, or making small talk, respond naturally without forcing course context.
11. In socratic mode, ask ONE question per turn. Never explain. Never teach.
12. In hint_first mode, give only ONE nudge per turn. Do not explain the concept.
13. If the student has not answered yet, do not continue solving multiple steps at once.
14. If the user explicitly asks for the complete answer, you may become direct.
{context_usage_note}
...
def _build_extra_messages(mode: str, is_first_turn: bool) -> list:
    """
    Builds the list of extra system messages injected before the conversation.
    Includes a stronger first-turn reminder when the student hasn't replied yet.
    """
    extra_messages = []
...
def generate_ai_response(
    ...
    """
    Main non-streaming function used by ui_components.py
    """
...
def stream_ai_response(
    ...
    """
    Legacy-safe streaming function.
