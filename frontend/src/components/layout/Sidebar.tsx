...
export default function Sidebar({
  ...
  onSelectChat, onNewChat, onDeleteChat, onLogout,
  ...
  return (
    <aside className="sidebar">
      {/* User card */}
      <div style={{
        background: "var(--orange-lt)", borderRadius: 14,
        padding: "1rem 1.1rem", border: "1px solid var(--orange-md)",
      ...
      {user.role === "student" && (
        <>
          ...
          <label className="label" style={{ marginTop: 10 }}>Teaching Mode</label>
          <select
            className="select"
            value={teachingMode}
          ...
          {filteredChats.length === 0 ? (
            <p style={{ color: "var(--text-soft)", fontSize: "0.82rem", textAlign: "center", padding: "0.5rem 0" }}>
              {Object.keys(chatMap).length === 0 ? "No chats yet — start learning! ✨" : "No chats found."}
            </p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {filteredChats.map(([id, chat]) => {
                const title = chat.title.length > 26 ? chat.title.slice(0, 26) + "…" : chat.title;
                ...
                return (
                  <div key={id} style={{ display: "flex", gap: 4 }}>
                    <button
                      ...
                      style={{
                        flex: 1, textAlign: "left", padding: "0.45rem 0.7rem",
                        borderRadius: "var(--r-sm)", border: "1.5px solid",
                    ...
                    <button
                      ...
                      style={{
                        padding: "0 0.5rem", borderRadius: "var(--r-sm)",
                        border: "1.5px solid var(--line)", background: "var(--bg2)",
                        color: "var(--text-soft)", cursor: "pointer", fontSize: "0.8rem",
                      }}
                    >
                      ✕
                    </button>
