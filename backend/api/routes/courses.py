...
@router.get("/")
def list_all_courses(_: dict = Depends(get_current_user)):
    return get_all_courses()


# ── GET /courses/mine ── öğretmenin kendi kursları
@router.get("/mine")
def list_my_courses(current_user: dict = Depends(require_teacher)):
    return get_teacher_courses(current_user["username"])
...
def create_new_course(
    ...
    success, message = create_course(body.course_name, current_user["username"])
    if not success:
...
def list_materials(course_id: str, _: dict = Depends(get_current_user)):
...
def delete_material(
    ...
    ok, msg, removed = delete_material_from_course(course_id, file_hash)
    if not ok:
